...
function RouteComponent() {
  const { data: products, isError: isProductsError } = useSuspenseQuery(
    productsApi.getProductsQueryOptions(false),
  )
  const { data: categories, isError: isCategoriesError } = useSuspenseQuery(
    productsApi.getCategoriesQueryOptions(),
  )
  const { data: collections, isError: isCollectionsError } = useSuspenseQuery(
    productsApi.getCollectionsQueryOptions(),
  )
  ...
  return (
    <FilteredTable combinedData={combinedData} configTable="productsTable" />
  )
}
